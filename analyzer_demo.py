...
@cross_origin()
def analyze_resume():
    """Main endpoint for resume analysis (demo version without database)"""
    try:
        ...
        return jsonify({
            ...
            'saved': False,  # Demo version doesn't save to database
            'demo_mode': True
        })
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'message': 'Resume analyzer demo is running'})
